...
function generateFlags(userAgent: string): Device {
  let mobile = false;
  ...
  const isDesktop = !mobileOrTablet;

  const preferCustomScrollbar = isDesktop && !macOS;
...
export const useDevice = () => {
  ...
  window.addEventListener("resize", () => {
    setTimeout(() => {
      const newFlags = generateFlags(navigator.userAgent);
