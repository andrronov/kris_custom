...
const app = new Hono();
const getStorageKey = (user, category) => `u:${user.id}:${category}`;
...
app.get("/profile", async (c) => {
    ...
    const [user, error] = await promiseResolver(db.select().from(schema.users).where(eq(schema.users.id, ctxUser.id)));
    if (error) {
        console.log(error);
...
app.put("/storage/:category/:key?", requireAuth, async (c) => {
    ...
    c.executionCtx.waitUntil(c.env.KC_KV.put(storageKey, JSON.stringify(finalValue)));
